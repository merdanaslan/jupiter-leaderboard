...
type RankBy = "livePnl" | "realizedPnl" | "volume" | "positionValue";
type Market = "SOL" | "ETH" | "BTC";

const DEFAULT_BASE_URL = "https://perps-api-v1.fly.dev/v1";
const RAW_USD_DECIMALS = 1_000_000;
const MARKET_MINTS: Record<Market, string> = {
...
interface CompetitionLeaderboardResponse {
  ...
  rankBy: RankBy;
  dataList: CompetitionLeaderboardEntry[];
}
...
async function pollOnce(input: {
  ...
  try {
    const { attempts, response } = await fetchCompetitionLeaderboardWithRetries(input.options, request);
    ...
    console.log("Jupiter Competition API Leaderboard");
    ...
    console.log("");
    console.log("This API path is expected to retry cleanly on transient 503/timeouts.");
  }
}

async function fetchCompetitionLeaderboardWithRetries(
  options: CliOptions,
  request: {
    walletAddresses: string[];
    startTimestamp: number;
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
      const response = await fetchCompetitionLeaderboard(options, request);
  ...
  throw new Error(
    `Competition API request failed after ${maxAttempts} attempt${maxAttempts === 1 ? "" : "s"}: ${formatUnknownError(lastError)}`,
  );
}
...
async function fetchCompetitionLeaderboard(
  ...
  try {
    const response = await fetch(`${trimBaseUrl(options.baseUrl)}/competition-leaderboard`, {
    ...
    return body as CompetitionLeaderboardResponse;
  } catch (error) {
    if (error instanceof Error && error.name === "AbortError") {
      throw new Error(`Competition API request timed out after ${options.requestTimeoutMs}ms`);
    }
    throw error;
  } finally {
...
function buildRequest(options: CliOptions, walletAddresses: string[]) {
...
function renderTerminalLeaderboard(input: {
  ...
  console.log(separator(width));
  console.log("Ctrl+C to stop. Values come from Jupiter competition API; equity is starting equity + live PnL.");
}
...
function parseArgs(args: string[]): CliOptions {
  const options: CliOptions = {
    baseUrl: process.env.PERPS_COMPETITION_API_URL || process.env.PERPS_API_URL || DEFAULT_BASE_URL,
    walletAddresses: [],
    sinceMinutes: 60,
    startingEquity: 100,
    rankBy: "livePnl",
...
function formatApiError(status: number, body: unknown): string {
  if (body && typeof body === "object") {
    ...
    const message = typeof record.message === "string" ? record.message : JSON.stringify(body);
    return `Competition API returned ${status}${code}: ${message}`;
  }

  return `Competition API returned ${status}${body ? `: ${String(body)}` : ""}`;
}
...
function rawUsdToNumber(value: string): number {
  const normalized = value.trim();
  if (!normalized) return 0;
  return Number(normalized) / RAW_USD_DECIMALS;
}
...
function printHelp() {
  console.log(`
Usage:
  npm run jupiter:api-leaderboard -- [options]

Examples:
  npm run jupiter:api-leaderboard -- --wallet <WALLET> --starting-equity 100 --max-polls 1
...
Notes:
  This tests Jupiter's HTTP competition API, not our Solstream/on-chain parser.
  The staging API can return transient 503 when its live position cache is unavailable.
`);
}
